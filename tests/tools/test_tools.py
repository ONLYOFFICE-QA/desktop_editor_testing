...
class TestTools:
    ...
    def check_open_files(self, files_dir: str) -> None:
    ...
    def check_open_desktop(self, retries: int = 5, timeout: int = 30) -> None:
        try_num = 1
        ...
        try:
            for _try in range(_retries):
                ...
                self._close_process_desktop(process)
                self.close_desktop()
                try_num += 1
    ...
    def _close_process_desktop(self, process: subprocess.Popen):
        try:
            process.terminate()
            process.wait(timeout=30) # TODO hangs
        except Exception as e:
            print(f"[red]|ERROR| Can't terminate process desktop. Error: {e}")

    def close_desktop(self, retries: int = 5, timeout: int = 5) -> bool:
        ...
        print(f"[red]|ERROR| Desktop close timeout has expired")
        return False
    ...
    def check_correct_version(self) -> None:
    ...
    def check_error_on_screen(self) -> None:
    ...
    def write_results(self, exit_code: str) -> None:
        self.report.write(
    ...
    def _create_display(self, visible: bool = False, size: tuple = (1920, 1080)) -> None:
    ...
    def _create_desktop(self, version: str = None) -> DesktopEditor:
    ...
    def _get_error_images(self) -> list:
        img_paths = FileUtils.get_paths(self.path.error_img_dir)
        return [Image.read(img_path=path) for path in img_paths]
